    ...
    try {
        ...
        if (action === 'export') {
            const [rows] = await db.execute(
                `SELECT app_type, contact_name, content, direction, FROM_UNIXTIME(timestamp/1000) as datetime
                 FROM conversations WHERE parent_email = ? ORDER BY timestamp DESC`, [email]
            );
        ...
        if (type === 'overview') {
            const [[stats]]    = await db.execute(
                `SELECT COUNT(*) as total_messages, SUM(CASE WHEN app_type='CALLS' THEN 1 ELSE 0 END) as total_calls
                 FROM conversations WHERE parent_email=? AND FROM_UNIXTIME(timestamp/1000)>=CURDATE()`, [email]
            );
            const [[locStats]] = await db.execute(
                `SELECT COUNT(*) as location_updates FROM locations WHERE parent_email=? AND FROM_UNIXTIME(timestamp/1000)>=CURDATE()`, [email]
            );
            const [devices] = await db.execute(
                `SELECT device_id, last_seen,
                        CASE WHEN last_seen >= DATE_SUB(NOW(), INTERVAL 3 MINUTE) THEN 'online' ELSE 'offline' END as status
                 FROM devices WHERE parent_email=? ORDER BY last_seen DESC`, [email]
            );
            return res.json({
        ...
        if (type === 'calls') {
            ...
            const [calls] = await db.execute(
                `SELECT contact_name, contact_id, direction, content, FROM_UNIXTIME(timestamp/1000) as datetime, timestamp
                 FROM conversations WHERE parent_email=? AND app_type='CALLS' AND timestamp BETWEEN ? AND ?
                 ORDER BY timestamp DESC LIMIT 500`, [email, start, end]
            );
        ...
        if (type === 'location' || type === 'locations') {
            ...
            const [locations] = await db.execute(
                `SELECT lat, lng, accuracy, FROM_UNIXTIME(timestamp/1000) as datetime, timestamp
                 ...
                 ORDER BY timestamp ASC LIMIT 1000`, [email, start, end]
            );
        ...
        if (type === 'browsing') {
            ...
            const [history] = await db.execute(
                `SELECT contact_name as title, content as url, FROM_UNIXTIME(timestamp/1000) as datetime, timestamp
                 ...
                 ORDER BY timestamp DESC LIMIT 500`, [email, since]
            );
        ...
        if (type === 'media') {
            const [media] = await db.execute(
                `SELECT app_type, contact_name, media_meta, FROM_UNIXTIME(timestamp/1000) as datetime, timestamp
                 ...
                 ORDER BY timestamp DESC LIMIT 200`, [email]
            );
